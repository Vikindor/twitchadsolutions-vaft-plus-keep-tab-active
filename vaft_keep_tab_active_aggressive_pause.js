...
(function() {
  ...
  function getPrototypeGetter(proto, prop) {
    try {
  ...
  function isManagedMediaElement(mediaElement) {
    if (!mediaElement || mediaElement.tagName !== 'VIDEO') {
      return false;
    }
    if (typeof mediaElement.pause !== 'function' || typeof mediaElement.play !== 'function') {
      return false;
    }
    if (typeof mediaElement.paused !== 'boolean') {
      return false;
    }
    try {
      return !!mediaElement.closest?.('[data-a-target="player-overlay"],[data-a-target="player-container"],.video-player');
    } catch {}
    return false;
  }
    ...
    if (keepAliveIntervalId === null) {
      keepAliveIntervalId = window.setInterval(() => {
        try {
          let moveEvent = null;
          try {
            moveEvent = new MouseEvent('mousemove', { bubbles: true });
          } catch {}
          if (!moveEvent) {
            moveEvent = new Event('mousemove', { bubbles: true });
          }
  ...
  function installAggressivePauseGuard() {
    if (pauseGuardInstalled) {
      return;
    }
    pauseGuardInstalled = true;
    const markGesture = () => {
    ...
    [
      ...
      window.addEventListener(eventName, markGesture, { capture: true, passive: true });
    });
    const HME = (window.HTMLMediaElement || HTMLMediaElement).prototype;
    const originalPause = HME.pause;
    const originalPlay = HME.play;
    const shouldAllowProgrammaticPause = () => {
      return (Date.now() - lastUserGesture) <= userGestureWindowMs;
    };
    const resumeIfPaused = (mediaElement) => {
      try {
        if (isManagedMediaElement(mediaElement) && mediaElement.paused && mediaElement.readyState > 2 && !mediaElement.ended) {
          const playPromise = originalPlay.call(mediaElement);
    ...
    try {
      const descriptor = Object.getOwnPropertyDescriptor(HME, 'pause');
      if (!descriptor?.value || !String(descriptor.value).includes(keepActivePatchTag)) {
        Object.defineProperty(HME, 'pause', {
          configurable: true,
          value: function vaftKeepTabActivePause() {
            if (!isManagedMediaElement(this)) {
    ...
    document.addEventListener('pause', (event) => {
      ...
      if (isManagedMediaElement(mediaElement) && !shouldAllowProgrammaticPause()) {
        try {
          event.stopImmediatePropagation();
        } catch {}
        resumeIfPaused(mediaElement);
      }
    }, true);
  ...
  function onContentLoaded() {
    ...
    const webkitHiddenGetter = getPrototypeGetter(docProto, 'webkitHidden');
    const mozHiddenGetter = getPrototypeGetter(docProto, 'mozHidden');
    try {
    ...
    const visibilityChange = e => {
      const videos = document.getElementsByTagName('video');
      if (videos.length > 0) {
        const documentWasHidden = (hiddenGetter && hiddenGetter.call(document) === true)
          || (webkitHiddenGetter && webkitHiddenGetter.call(document) === true)
          || (mozHiddenGetter && mozHiddenGetter.call(document) === true);
